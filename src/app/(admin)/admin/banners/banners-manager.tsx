...
export function BannersManager({ banners: inicial }: { banners: any[] }) {
  ...
  const [draft, setDraft] = useState<{ titulo: string; link: string; bg: string; ativo: boolean }>({
    titulo: "", link: "", bg: CORES[0][1], ativo: false
  });
  ...
  function alternar(id: string, ativo: boolean) {
    ...
    startTransition(async () => {
      ...
      const res = await alternarAtivoBanner(id, ativo).catch(() => ({ ok: false }));
      if (!res.ok) {
        trocar(ativo);
